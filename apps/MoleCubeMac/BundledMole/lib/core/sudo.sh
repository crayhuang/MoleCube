...
request_sudo_access() {
    ...
    # Detect if running in a usable TTY environment. GUI apps may still see a
    # readable /dev/tty path, but opening it can fail with "Device not
    # configured" when there is no controlling terminal.
    local tty_path=""
    ...
    if [[ ! -r "$tty_path" || ! -w "$tty_path" ]] ||
        ! { : < "$tty_path" > "$tty_path"; } 2> /dev/null; then
        is_gui_mode=true
    fi
