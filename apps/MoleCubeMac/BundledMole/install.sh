...
download_binary() {
    ...
    local had_existing_binary=false
    ...
    install_verified_download() {
        ...
        tmp_binary="$(mktemp "${TMPDIR:-/tmp}/mole-${binary_name}.XXXXXX")" || return 1

        if [[ -t 1 ]]; then
            start_line_spinner "$label"
        else
            echo "$label"
        fi

        if curl -fsSL --connect-timeout 10 --max-time 60 -o "$tmp_binary" "$download_url"; then
            if [[ -t 1 ]]; then stop_line_spinner; fi
            if verify_release_asset_checksum "$tag" "$asset_name" "$tmp_binary"; then
                chmod +x "$tmp_binary"
                xattr -c "$tmp_binary" 2> /dev/null || true
                mv -f "$tmp_binary" "$target_path"
                log_success "Downloaded ${binary_name} binary"
                return 0
            fi
            rm -f "$tmp_binary"
            log_warning "Checksum verification failed for ${binary_name} from ${tag}"
            return 1
        fi
    ...
    install_built_binary() {
        local tmp_binary
        tmp_binary="$(mktemp "${TMPDIR:-/tmp}/mole-${binary_name}-build.XXXXXX")" || return 1
        if build_binary_from_source "$binary_name" "$tmp_binary"; then
            chmod +x "$tmp_binary"
            mv -f "$tmp_binary" "$target_path"
            return 0
        fi
    ...
    if install_verified_download "$release_tag" "$url" "Downloading ${binary_name}..."; then
        return 0
    fi
    ...
    if [[ -n "$fallback_tag" && "$fallback_tag" != "$release_tag" ]]; then
        ...
        if install_verified_download "$fallback_tag" "$fallback_url" "Retrying ${binary_name} from ${fallback_tag}..."; then
    ...
    if install_built_binary; then
        return 0
    fi
    if [[ "$had_existing_binary" == "true" ]]; then
        log_warning "Keeping existing ${binary_name} binary because the replacement could not be verified"
        return 0
    fi
